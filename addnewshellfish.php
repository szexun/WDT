<?php
    ...
    if(isset($_POST['addFish'])){
        ...
        $shellfishName = $_POST['txtShellFishName'];
        $shellfishDescription = $_POST['txtDescription'];
        $shellfishPrice = $_POST['txtPrice'];
        $shellfishPicture = $_POST['txtPicture'];
        $sellerContact = $_POST['txtSellerContact'];

        // query to insert data into database
        $sql_query = "INSERT INTO `shellfish`(`shellfish_name`, `shellfish_description`, `shellfish_price`, `shellfish_picture`,`user_id`, `seller_contact`) 
        VALUES ('$shellfishName','$shellfishDescription','$shellfishPrice','$shellfishPicture', '$user_id','$sellerContact')";
        
         // if sql query is executed successfully
        if(mysqli_query($conn,  $sql_query)){
            echo "<script>alert('Add Successfully')</script>";
            echo "<script> window.location.href='shellfish.php';</script>";
        } else {
            echo "Add Failed.";
        }
    } 
...
<body>
    ...
    <body>
        ...
        <h1>Add New Shellfish</h1>
    </div>
    <div class="container">
        <form class="row g-3" action="" method="post">
            <div class="col-md-6">
                <label for="inputEmail4" class="form-label">Shell Fish Name:</label>
                <input type="text" class="form-control" name="txtShellFishName" required>
            </div>
            <div class="col-md-6">
                <label for="inputPassword4" class="form-label">Shell Fish Description:</label>
            ...
            <div class="col-12">
                <button type="submit" class="btn btn-primary" name="addFish">Add Shellfish</button>
            </div>
        </form>
    </div>
    </body>
